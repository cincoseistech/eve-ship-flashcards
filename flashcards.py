...
BUILTIN_PRESETS = {
    "Visual ID — Image to Name": {
        "show_image_front": True, "front_fields": [],
        "show_image_back": False, "back_fields": ["typeName", "groupName"],
        "ship_classes": "__all__", "res_as_percent": True, "builtin": True
    },
    "Name ID — Name to Image": {
        "show_image_front": False, "front_fields": ["typeName"],
        "show_image_back": True, "back_fields": ["groupName"],
        "ship_classes": "__all__", "res_as_percent": True, "builtin": True
    },
    "Fitting Knowledge — Name to Slots": {
        "show_image_front": False, "front_fields": ["typeName", "groupName"],
...
if selected_preset != "Manual" and selected_preset in all_presets:
    if st.sidebar.button("Apply Preset"):
        st.session_state['_preset_applied'] = all_presets[selected_preset]
        st.rerun()
...
selected_classes = st.sidebar.multiselect(
    "Ship Classes", options=all_classes,
    default=st.session_state.get('w_ship_classes', all_classes),
    key='w_ship_classes', label_visibility="collapsed"
)
...
show_image_front = st.sidebar.checkbox(
    "Show ship image", value=st.session_state.get('w_image_front', True), key='w_image_front'
)
front_fields = st.sidebar.multiselect(
    "Front data fields", options=AVAILABLE_COLS,
    default=st.session_state.get('w_front_fields', safe_cols(['typeName'])),
    key='w_front_fields', label_visibility="collapsed"
)

st.sidebar.markdown("---")
st.sidebar.markdown('<div class="sidebar-section">Back of Card</div>', unsafe_allow_html=True)
show_image_back = st.sidebar.checkbox(
    "Show ship image", value=st.session_state.get('w_image_back', False), key='w_image_back'
)
back_fields = st.sidebar.multiselect(
    "Back data fields", options=AVAILABLE_COLS,
    default=st.session_state.get('w_back_fields', safe_cols(['groupName'])),
    key='w_back_fields', label_visibility="collapsed"
)
...
res_as_percent = st.sidebar.checkbox(
    ...
    value=st.session_state.get('w_res_percent', True), key='w_res_percent'
)
...
if st.sidebar.button("Save Preset"):
    if new_preset_name.strip():
        ...
        custom[new_preset_name.strip()] = {
            "show_image_front": show_image_front, "front_fields": front_fields,
            "show_image_back": show_image_back, "back_fields": back_fields,
            "ship_classes": selected_classes if set(selected_classes) != set(all_classes) else "__all__",
            "res_as_percent": res_as_percent, "builtin": False
        }
